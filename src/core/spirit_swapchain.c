...
SpiritResult
createDepthObjects(const SpiritDevice device, SpiritSwapchain swapchain);
    ...
    SpiritSwapchain optionalSwapchain)
{
    ...
    if (!createInfo->selectedFormat)
    {
        ...
        createInfo->preferedFormat.colorSpace =
    ...
    const u32 maxImageCount =
        device->swapchainDetails.capabilties.maxImageCount;
    const u32 minImageCount =
        device->swapchainDetails.capabilties.minImageCount;
    const u32 swapImageCount = clamp_value(3, minImageCount, maxImageCount);

    swapInfo.minImageCount = swapImageCount;

    // output info
    VkExtent2D outExtent = {createInfo->windowRes.w, createInfo->windowRes.h};
    ...
    swapInfo.preTransform =
        device->swapchainDetails.capabilties.currentTransform;
...
SpiritResult createImages(const SpiritDevice device, SpiritSwapchain swapchain)
{
    ...
    vkGetSwapchainImagesKHR(
        device->device, swapchain->swapchain, &swapchain->imageCount, NULL);

    // allocate new array of images, and populate it
    VkImage imageBuf[swapchain->imageCount];
    vkGetSwapchainImagesKHR(
        device->device, swapchain->swapchain, &swapchain->imageCount, imageBuf);
...
createDepthObjects(const SpiritDevice device, SpiritSwapchain swapchain)
{
    ...
    swapchain->depthImages = new_array(SpiritImage, swapchain->imageCount);
    ...
    VkSurfaceFormatKHR preferedFormat)
{
    ...
    {
        if (availableFormats[i].format == preferedFormat.format &&
